...
class AddNewMoreGroupMember extends StatefulWidget {
  GroupChatModel groupChatModel;

  var newMembers;
  var roomids;
  var jsondata;
  AddNewMoreGroupMember({
    this.groupChatModel,
    this.newMembers,
    this.roomids,
    this.jsondata
  ...
  @override
  _AddNewMoreGroupMemberState createState() => _AddNewMoreGroupMemberState();
}

class _AddNewMoreGroupMemberState extends State<AddNewMoreGroupMember> {
                      ...
                      child: ListTile(
                        ...
                        trailing:  Container(
                          ...
                          child: FlatButton(
                              color: KBlueColor,
                              onPressed: (){
                                  print(widget.roomids.toString());
                                    Collection.groupChatRoom.doc(widget.roomids.toString())
                                    .update({"members": FieldValue.arrayUnion(controller.addGroupMember)}).whenComplete(() {
                                    }).whenComplete(() {
                                      print(controller.addGroupMember);

                                      for(int i=0;i<controller.addGroupMember.length;i++){
                                        Collection.blockGroupChatUser.where("user",isEqualTo: controller.addGroupMember[i])
                                            .where("room_id").get().then((value) {
                                             var docs = value.docs;
                                              if(docs.length>0){
                                                print("check the detail");
                                                print(docs.length);
                                                print(docs.single.id);
                                                Collection.blockGroupChatUser.doc(docs.single.id)
                                                    .update({
                              ...
                              Container(
                                ...
                                child: Card(
                                  ...
                                  child: TextField(
                                    onChanged: (value) {
                                      setState(() {
                                        searchString4 = value.replaceAll(" ", "").toLowerCase();
                                      });
                                    },
  ...
  Widget makeFriend(BuildContext context, DocumentSnapshot documnents) {
    ...
    if (toName.contains(searchString4)) {
